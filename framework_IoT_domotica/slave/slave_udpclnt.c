...
int main ()
{
   ...
   do 
   {
      ...
      do
      {
         if ((received_msg_size = recvfrom(sk, udp_response, SIZE, 0, (struct sockaddr*) &fromAddr, &fromSize) <= 0) || strlen (udp_response) == 0)
         {
            printf ("nothing was received, waiting for 5 seconds...\n");
            sleep (5);
         }
      } while (received_msg_size <= 0 || strlen (udp_response) == 0);
