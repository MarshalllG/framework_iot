...
void TCPserver (void)
{
   ...
   {         
      ...
      printf("connection accepted, client nr° %d has ip %s port %d\n", i, inet_ntoa(caddr.sin_addr), caddr.sin_port);
        
      // funzione per verificare lo stato di ogni dispositivo
      multithread_f();
...
// riceve l'indice di un thread e avvia una comunicazione UDP
...
***/
   ...
   // assegna gli stati
   if ((ping_res = udp_ping (Srv, fromAddr, sk, index)) == 1)
   {
...
int udp_ping (struct sockaddr_in Srv, struct sockaddr_in fromAddr, int ping_sk, int i)
{
   char msg_to_send[SIZE] = "Qual è la temperatura rilevata dal sensore?";
...
***/
   ...
   if (sendto(ping_sk, msg_to_send, sizeof(msg_to_send), 0, (struct sockaddr *) &Srv, sizeof(Srv)) < 0)
   {
